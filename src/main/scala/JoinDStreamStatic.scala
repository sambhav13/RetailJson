...
object JoinDStreamStatic {
  ...
  def main(args: Array[String]) {
   ...
   val staticData =   dataSet.rdd.map(x => {
    
     (x(0).toString(), Record (x(0).toString().toInt,x(1).toString(),x(2).toString()))
    ...
    val newDstr =  lines.map(w => {
        val words = w.split(",")
        (words(0), Record(words(0).toInt, words(1), words(2)))
        }
    )
     // Join each batch in stream with the dataset
  val joinedstr = newDstr.transform(x => staticData.join(x))
  
   joinedstr.foreachRDD(x =>
     {
         x.collect().foreach(p=>println("joined rdd"+p._2._2))
     }
   )
